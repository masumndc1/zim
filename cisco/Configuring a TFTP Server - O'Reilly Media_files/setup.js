...
if (typeof mybuys.setupJsRun == "undefined")
{
	...
	mybuys.initPage = function()
	{
		if((this.params["pt"]) && (this.params["pt"].indexOf("purchase") != -1))
		{
			...
			{
				var testemail = Math.max(this.params['email'].toUpperCase().indexOf("@OREILLY.COM")
									    ,this.params['email'].toUpperCase().indexOf("@OREILLY.CO.UK")
										,this.params['email'].toUpperCase().indexOf("REALTIMEQATEST+QATEST2@")
										,this.params['email'].toUpperCase().indexOf("FERIHARJULIANTO@GMAIL.COM"));
				if(testemail<0)
				{
					this.base_initPage();
				}
